...
def fetch_twse_data():
    try:
        ...
        if len(fund_res) > 0 and 'ForeignInvestorBuySell' in fund_res[0]:
...
if not raw_df.empty:
    ...
    if not display_df.empty:
        ...
        with st.spinner(f'正在抓取 {selected_stock} 的歷史資料與計算指標...'):
            # 使用 yfinance 抓取近半年資料 (加上 .TW 後綴代表台灣上市)
            ...
            if not hist.empty:
                ...
                st.warning("無法抓取該標的的歷史 K 線資料 (可能為剛上市或代號不相符)。")
else:
